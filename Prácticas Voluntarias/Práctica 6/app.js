...
//Creación de un middleware de control de acceso
...
app.post("/addTask", middlewareCurrentUser, function(request, response){
    let task = utils.createTask(request.body.taskHidden);
...
//Manejador de ruta para la imagen de perfil (añadir middlewareCurrentUser si queremos usar el email del usuario)
app.get("/imagenUsuario", middlewareCurrentUser, function(request, response){
    daoU.getUserImageName(response.locals.userEmail, function(err, imagenDevuelta){
        if(err){
            ...
            response.sendFile(path.join(__dirname, "profile_imgs", imagenDevuelta));
        }
        else{
